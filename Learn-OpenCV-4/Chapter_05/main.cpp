...
static Scalar randomColor( RNG& rng )
{
	...
	cout << "randomColor icolor: " << icolor << endl;
 ...
 * 这个基本函数通过使用相对于图像大小的大内核尺寸将模糊效果应用于输入图像，在代码中，是原始的三分之一。
 */
...
// ./Chapter5 ../data/test.pgm lightMethod 0 segMethod 1
int main( int argc, const char** argv )
{
	...
	cout << " img file: " << img_file.c_str()<< "\n";
	cout << " light_pattern file: " << light_pattern_file.c_str() << "\n";
	...
	// Remove noise
	Mat img_noise, img_box_smooth, img_goss, img_blur;
	GaussianBlur(img, img_goss, Size(3, 3), 0, 0, cv::BORDER_REPLICATE);
	...
	if(light_pattern.data == NULL){
		...
		cout << "light pattern data is NULL, calculateLightPattern. \n";
		light_pattern = calculateLightPattern(img_noise);
		imshow("pattern", light_pattern);
